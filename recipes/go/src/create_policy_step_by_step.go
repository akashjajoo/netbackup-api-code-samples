...
func main() {
    ...
    jwt := authentication.Login(*nbmaster, httpClient, *username, *password, *domainName, *domainType)

    policies.CreatePolicyWithDefaults(*nbmaster, httpClient, jwt)
    policies.ListPolicies(*nbmaster, httpClient, jwt)
    policies.ReadPolicy(*nbmaster, httpClient, jwt)
    policies.AddClient(*nbmaster, httpClient, jwt)
    policies.AddSchedule(*nbmaster, httpClient, jwt)
    policies.AddBackupSelection(*nbmaster, httpClient, jwt)
    policies.ReadPolicy(*nbmaster, httpClient, jwt)
    policies.DeleteClient(*nbmaster, httpClient, jwt)
    policies.DeleteSchedule(*nbmaster, httpClient, jwt)
    policies.ReadPolicy(*nbmaster, httpClient, jwt)
    policies.DeletePolicy(*nbmaster, httpClient, jwt)
    policies.ListPolicies(*nbmaster, httpClient, jwt)
}
